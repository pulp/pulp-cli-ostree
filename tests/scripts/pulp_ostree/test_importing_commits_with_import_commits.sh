...
cleanup() {
  pulp ostree repository destroy --name "cli_test_ostree_import_commits_repository" || true
  pulp ostree distribution destroy --name "cli_test_ostree_import_commits_distro" || true
...
expect_succ pulp ostree repository create --name "cli_test_ostree_import_commits_repository"

if pulp debug has-plugin --name "ostree" --specifier ">=2.0.0"
then
  expect_succ pulp ostree repository import-all --name "cli_test_ostree_import_commits_repository" \
    --file "fixtures_small_repo.tar" \
    --repository_name "small"
else
  expect_succ pulp ostree repository import-commits --name "cli_test_ostree_import_commits_repository" \
    --file "fixtures_small_repo.tar" \
    --repository_name "small"
fi
...
expect_succ pulp ostree distribution create --name "cli_test_ostree_import_commits_distro" \
  --base-path "cli_test_ostree_import_commits_distro" \
  --repository "cli_test_ostree_import_commits_repository"
